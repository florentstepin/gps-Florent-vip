...
def afficher_rappel_projet():
    """Affiche un résumé du projet en haut des pages 2 et 3"""
    if st.session_state.project_data["idea"]:
        st.info(f"📌 **Projet Initial :** {st.session_state.project_data['idea']}")
        if st.session_state.project_data["analysis"]:
            with st.expander("📄 Voir le détail de l'Audit Phase 1 (Obstacles & Verdict)"):
...
# PHASE 2
    ...
    # RAPPEL CONTEXTE (Votre demande)
    afficher_rappel_projet()
    st.divider()
