...
st.markdown("""
    ...
    /* Navigation Bandeau */
    .nav-container { display: flex; justify-content: center; gap: 10px; margin-bottom: 20px; }
    .nav-step { padding: 10px 20px; border-radius: 20px; background: #262730; color: #7f5af0; font-weight: bold; border: 1px solid #7f5af0; cursor: pointer; }
    .nav-active { background: #7f5af0; color: white; }
...
def create_pdf_bytes(data):
    pdf = FPDF()
    pdf.add_page()
    # Gestion des polices standards pour éviter les erreurs d'encodage
    ...
    for title, content in sections:
        ...
        text_content = content if content else "Non généré"
        pdf.multi_cell(0, 5, text_content.encode('latin-1', 'replace').decode('latin-1'))
        pdf.ln(5)
    
    # Sortie directe en bytes pour Streamlit
    ...
    if st.session_state.user:
        ...
        with st.expander("📂 Gestion de Session", expanded=False):
            ...
            if st.session_state.project["analysis"]:
                ...
                st.download_button("📄 Télécharger le Rapport PDF", pdf_data, "Rapport_Stratege.pdf", "application/pdf", use_container_width=True)
            ...
            if up:
                if st.button("✅ Valider l'Import"):
                    ...
                    # On recalcule l'étape en fonction des données présentes
                    if data.get("gps"): st.session_state.current_step = 3
                    elif data.get("pivots"): st.session_state.current_step = 2
                    else: st.session_state.current_step = 1
                    st.rerun()

        # HIGH TICKET DÉBLOQUÉ DÈS L'ANALYSE
        with st.expander("💎 Expertise Humaine", expanded=True):
            if st.session_state.project["analysis"]:
                st.markdown("<p style='font-size:0.85em;'>Besoin d'un regard expert ? Envoyez votre dossier à Florent.</p>", unsafe_allow_html=True)
                msg_exp = st.text_area("Votre message pour Florent :", placeholder="Questions, demande de coaching...")
                if st.button("🚀 Envoyer mon dossier à Florent"):
                    with st.spinner("Envoi..."):
                        pdf_report = create_pdf_bytes(st.session_state.project)
                        if send_audit_email(msg_exp, pdf_report): st.success("Dossier envoyé !")
...
st.markdown("<div class='intro-box'><b>Bienvenue dans votre Usine à Stratégie.</b> Suivez les 3 étapes ou sollicitez Florent pour un audit approfondi.</div>", unsafe_allow_html=True)
...
if st.session_state.current_step == 1:
    ...
    if st.session_state.project["analysis"]:
        ...
        ctx = c2.text_area("Contexte (Cible, budget) :", height=150, placeholder="Solo-preneur, 2000€...")
